...
function ServiceDetails({ service, onClose }: { service: Service; onClose: () => void }) {
  ...
  return <div role="dialog" aria-modal="true" aria-label={`${title(service.service)} details`} style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(2,6,23,.62)', display: 'flex', justifyContent: 'flex-end' }} onClick={onClose}>
    <aside onClick={(event) => event.stopPropagation()} style={{ width: 'min(560px, 100%)', height: '100%', overflowY: 'auto', background: 'var(--c1)', borderLeft: '1px solid var(--bd)', padding: 22, boxShadow: '-12px 0 40px rgba(0,0,0,.25)' }}>
      ...
      <section style={{ marginTop: 20, paddingBottom: 20 }}><h3 style={{ color: 'var(--t1)', fontSize: 13, margin: '0 0 10px' }}>Configuration</h3><div style={{ color: 'var(--t2)', fontSize: 12 }}>Status: <strong>{service.metadata?.configurationOnly ? 'Configuration check only' : 'Configuration status not exposed'}</strong><br /><span style={{ color: 'var(--t3)' }}>Secret values and connection strings are never displayed.</span></div></section>
    </aside>
  </div>;
}
...
export default function SystemCheckPage() {
  ...
  const { data, isLoading, isFetching, refetch } = useQuery({ queryKey: ['monitoring-status'], queryFn: async () => (await api.get('/monitoring/status')).data.data, refetchInterval: 30000 });
  ...
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><Link href="/admin" style={{ display: 'grid', placeItems: 'center', width: 34, height: 34, borderRadius: 8, background: 'var(--bd)', color: 'var(--t1)' }}><ArrowLeft size={18} /></Link><div><h1 className="pg-h" style={{ fontSize: 24, marginBottom: 4 }}>System Check</h1><p className="pg-sh">Operational health, incidents, performance, and disaster-recovery status</p></div></div><button onClick={() => refetch()} disabled={isFetching} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 14px', borderRadius: 8, border: '1px solid var(--bd)', background: 'transparent', color: 'var(--t1)', cursor: isFetching ? 'wait' : 'pointer' }}>{isFetching ? <Loader2 size={14} className="animate-spin" /> : <RefreshCw size={14} />} Refresh</button></div>
    ...
    {isLoading ? <div style={{ padding: 40, textAlign: 'center', color: 'var(--t2)' }}><Loader2 size={30} className="animate-spin" /> Checking services...</div> : <section style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(285px,1fr))', gap: 14 }}>{services.map((service) => <article key={service.service} onClick={() => setSelectedService(service)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') setSelectedService(service); }} role="button" tabIndex={0} aria-label={`View ${title(service.service)} details`} style={{ padding: 18, borderRadius: 12, border: '1px solid var(--bd)', background: 'var(--c2)', cursor: 'pointer' }}><div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center' }}><div style={{ display: 'flex', alignItems: 'center', gap: 9, color: 'var(--t1)' }}>{icons[service.service] || <Activity size={18} />}<strong style={{ fontSize: 13 }}>{title(service.service)}</strong></div><StatusBadge status={service.status} /></div><p style={{ color: 'var(--t3)', fontSize: 12, lineHeight: 1.5, margin: '12px 0 0' }}>{service.errorMessage || (service.status === 'ONLINE' ? 'Last check succeeded · View details' : 'View failure details')}</p></article>)}</section>}
