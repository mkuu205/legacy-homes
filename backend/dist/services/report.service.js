...
class ReportService {
    async getBillingReport(query) {
        const where = {};
        if (query.startMonth)
            where.billingMonth = { gte: query.startMonth };
        if (query.endMonth)
            where.billingMonth = { ...where.billingMonth, lte: query.endMonth };
    ...
    async getRevenueReport(query) {
        ...
        const paymentsWithResident = await Promise.all(payments.map(async (p) => {
    ...
    async getOverdueReport() {
        ...
        const billsWithDetails = await Promise.all(overdueBills.map(async (bill) => {
            const [resident, meter, house] = await Promise.all([
                prisma_1.default.user.findUnique({
                    where: { id: bill.residentId },
    ...
    async getConsumptionReport(query) {
        ...
        const readingsWithDetails = await Promise.all(readings.map(async (reading) => {
            ...
            const house = meter?.houseId
                ? await prisma_1.default.house.findUnique({ where: { id: meter.houseId } })
                : null;
    ...
    async getAdminDashboardStats() {
        ...
        const [totalResidents, activeResidents, totalBills, paidBills, unpaidBills, overdueBills, monthlyRevenue, lastMonthRevenue, pendingPayments, successfulPayments, openTickets, totalMeters,] = await Promise.all([
            prisma_1.default.user.count({ where: { role: 'RESIDENT' } }),
            prisma_1.default.user.count({ where: { role: 'RESIDENT', accountStatus: 'ACTIVE' } }),
            prisma_1.default.bill.count(),
            prisma_1.default.bill.count({ where: { status: 'PAID' } }),
            prisma_1.default.bill.count({ where: { status: { in: ['UNPAID', 'PARTIAL'] } } }),
            prisma_1.default.bill.count({ where: { status: 'OVERDUE' } }),
            ...
            prisma_1.default.ticket.count({ where: { status: { in: ['OPEN', 'PENDING'] } } }),
            prisma_1.default.meter.count({ where: { status: 'ACTIVE' } }),
        ]);
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            const rev = await prisma_1.default.payment.aggregate({
