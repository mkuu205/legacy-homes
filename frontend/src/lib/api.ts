...
export const checkBackendHealth = async (): Promise<HealthResponse> => {
  ...
  try {
    ...
    store.recordSuccessfulConnection(latency, healthData.version, {
      databaseStatus: healthData.database || 'UNKNOWN',
      smtpStatus: healthData.smtp || 'UNKNOWN',
      tumaStatus: healthData.tuma || 'UNKNOWN',
      pesapalStatus: healthData.pesapal || 'UNKNOWN',
    });
