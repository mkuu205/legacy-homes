...
export default function DashboardPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }} className="fu">
      ...
      <div style={{
        background: 'linear-gradient(135deg, rgba(0, 198, 167, 0.15), rgba(0, 198, 167, 0.05))',
        ...
        <div style={{ flex: 1 }}>
          <p style={{ fontSize: '13px', fontWeight: 600, color: 'var(--t1)', marginBottom: '2px' }}>
            Welcome back, {user?.fullName?.split(' ')[0]}!
          </p>
          <p style={{ fontSize: '12px', color: 'var(--t2)' }}>
            You have {unreadNotifications || 0} unread alerts. Check your notifications for important updates.
          </p>
        </div>
      </div>
      ...
      {currentBill && (
        <div className="pay-hero">
          <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <div>
              <p style={{ fontSize: '12px', color: 'rgba(0, 198, 167, 0.8)', fontWeight: 600, marginBottom: '8px' }}>
                Current Bill — {currentBill.billingMonth}
              </p>
              <p style={{ fontSize: '32px', fontWeight: 800, color: 'var(--t1)', fontFamily: 'var(--f1)', lineHeight: 1 }}>
                KES {currentBill.totalAmount.toLocaleString()}
              </p>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '12px' }}>
                <span style={{ fontSize: '12px', color: 'rgba(0, 198, 167, 0.8)' }}>
                  Balance: KES {currentBill.balance.toLocaleString()}
                </span>
                <div
              ...
              <p style={{ fontSize: '11px', color: 'rgba(0, 198, 167, 0.7)', marginTop: '8px' }}>
                Due: {new Date(currentBill.dueDate).toLocaleDateString('en-KE')}
              </p>
            </div>
      ...
      {!currentBill && (
        <div className="card" style={{ textAlign: 'center', padding: '32px 20px' }}>
          <CheckCircle size={40} style={{ color: 'var(--ok)', margin: '0 auto 12px', display: 'block' }} />
